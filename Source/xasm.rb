...
module Assembly
  ...
  module Library
    ...
    class Library
      ...
      def self.load(filename)
        ...
        code_sect = library.sections.find{|x|x.flag == Section::CODE}
  ...
  class Parser
    ...
    def load_lib(filename)
      @libraries << Library::Library.load(filename)
    end
    def parse(code)
      @line_num = 0
...
a = parser.libraries.map{|x|x.generate_code}.join
b = result[:object].image
puts "encodings: a => #{a.encoding}, b => #{b.encoding}"
image = a + b
